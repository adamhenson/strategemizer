...
import { getBarsWithRetry, getTechnicalIndicatorsInput } from '../../lib/utils';
import { dojiCreator } from '../../patterns/doji';
import { Asset } from '../../types';

const doji = dojiCreator({ bodyPercentOfRange: 10 });

const MAX_AVG_DAILY_VOLUME = 5000000;
const MIN_AVG_DAILY_VOLUME = 500000;
const MIN_STOCK_PRICE = 70;
...
const validationDoji = async ({
  ...
  if (
    !doji(
      getTechnicalIndicatorsInput([bars1Year[bars1Year.length - 1]]),
      asset.symbol,
    )
  ) {
    return false;
